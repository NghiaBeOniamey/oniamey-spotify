...
public interface AdminSongRepository extends SongRepository {
    ...
    @Query(value = """
        ...
        AND (:#{#request.genre} IS NULL OR gs.genre_id IN (:#{#request.genre}))
        AND (:#{#request.status} IS NULL OR s.status = :#{#request.status})
        GROUP BY s.created_date, s.title, s.id, s.artist, s.release_date, s.status
        """, countQuery = """
        SELECT count(DISTINCT s.created_date)
            FROM song s
            LEFT JOIN genre_song gs ON gs.song_id = s.id
            LEFT JOIN genre g ON g.id = gs.genre_id
        WHERE
